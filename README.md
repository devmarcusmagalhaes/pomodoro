...
> **AÇÃO NECESSÁRIA:** preencha os nomes antes de entregar.

| Nome | Atividades desenvolvidas |
|------|--------------------------|
| _Integrante 1_ | _ex.: arquitetura do `AuthService`, hash SHA-256, fluxo de splash_ |
| _Integrante 2_ | _ex.: integração do Hive, `SessaoService`, tela de perfil_ |
| _Integrante 3_ | _ex.: UI da tela de login, validações, melhorias de UX_ |
...
│   └── tela_debug.dart                ← inspetor do banco (apresentação)
│
...
| **Inspetor do banco de dados** | `views/tela_debug.dart` | Tela que mostra o conteúdo cru do Hive + SharedPreferences em JSON. Útil para apresentação e debug. Acesso: Perfil → ícone de "storage". |
